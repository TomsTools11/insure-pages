...
- `tests/*.test.mjs` — 41 `node:test` assertions against the built HTML. The shared `page()` helper lives in `tests/support/page.mjs`, which is imported by every test file. **Test files must never import from each other**: `node --test` globs `tests/*.mjs`, so a test file that imports another test file re-registers that file's tests, silently duplicating them. Keep shared helpers under `tests/support/` (outside the glob) and have each test file import only from there.
...
The site is live at **`https://www.insurepages.com`**. That exact origin — `www`,
no trailing slash — is the canonical host, and it is set in ONE place:

- `site/astro.config.mjs` -> `site:`. It drives `<link rel="canonical">`, `og:url`,
  `og:image`, `twitter:image` (all via `Astro.site` in `src/layouts/Base.astro`)
  and every `<loc>` in the generated sitemap. Change it there, never per-page.
- `site/public/robots.txt` is the one exception: it is copied verbatim from
  `public/`, so its `Sitemap:` line needs its own edit whenever the origin changes.

`https://insurepages.com` (bare apex) 308-redirects to `www` at the Vercel domain
level, so `www` is the only host that serves 200 — never point a canonical, a
sitemap `<loc>`, or an internal absolute link at the apex.

`https://insure-pages.vercel.app` still serves the same site and is publicly
crawlable. The corrected cross-domain `rel="canonical"` is what consolidates it
into `www.insurepages.com`. A host-scoped 308 on that alias would consolidate it
harder, but it is **sequenced, not optional-forever**: campaign email already in
recipients' inboxes hot-links two images from that host (the logo and the
image-only fallback PNG), and mail image proxies handle 308 poorly, so redirect
only once those campaigns have aged out. Never redirect or retire it before then,
and never match a `*.vercel.app` wildcard — that would catch preview deployments.
(The other `*.vercel.app` aliases are behind Vercel SSO and already send
`x-robots-tag: noindex`, so they need nothing.)

`site/tests/canonical.test.mjs` locks all of this in and will fail the build if
the deployment URL ever ships again.

Still open, per PRD §17.3: the **product platform** stack — the templated, multi-tenant engine that will generate client agencies' sites — is undecided ("which CMS/stack and hosting; build vs. buy for the template engine and connectors"). If asked to scaffold or implement that platform, surface the stack decision first rather than picking silently; do not assume a framework for it. This is separate from the marketing-site stack above, which is now fixed. (The custom domain is no longer open — the site is live at
`https://www.insurepages.com`; see "Canonical origin" above. The header logo is no longer open — the
